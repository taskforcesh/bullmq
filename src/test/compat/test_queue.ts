...
describe('Queue', () => {
  ...
  describe('.close', () => {
    ...
    it('should resolve the promise when each client has disconnected', () => {
      ...
      return testQueue.close().then(() => {
        expect(testQueue.client.status).to.be.equal('end');
        expect(testQueue.eclient.status).to.be.equal('end');
      });
    });
  ...
  describe('instantiation', () => {
    it('should create a queue with standard redis opts', done => {
      ...
      expect((queue.client as any).options.host).to.be.equal('127.0.0.1');
      expect((queue.eclient as any).options.host).to.be.equal('127.0.0.1');

      expect((queue.client as any).options.port).to.be.equal(6379);
      expect((queue.eclient as any).options.port).to.be.equal(6379);
    ...
    it('should create a queue with a redis connection string', () => {
      ...
      expect((queue.client as any).options.host).to.be.equal('123.4.5.67');
      expect((queue.eclient as any).options.host).to.be.equal('123.4.5.67');

      expect((queue.client as any).options.port).to.be.equal(1234);
      expect((queue.eclient as any).options.port).to.be.equal(1234);

      expect((queue.client as any).options.db).to.be.equal(2);
      expect((queue.eclient as any).options.db).to.be.equal(2);
    ...
    it('should create a queue with only a hostname', () => {
      ...
      expect((queue.client as any).options.host).to.be.equal('127.2.3.4');
      expect((queue.eclient as any).options.host).to.be.equal('127.2.3.4');

      expect((queue.client as any).options.port).to.be.equal(6379);
      expect((queue.eclient as any).options.port).to.be.equal(6379);

      expect((queue.client as any).condition.select).to.be.equal(0);
      expect((queue.eclient as any).condition.select).to.be.equal(0);
    ...
    it('should create a queue with connection string and password', () => {
      ...
      expect((queue.client as any).options.password).to.be.equal('123');
      expect((queue.eclient as any).options.password).to.be.equal('123');
    ...
    it('creates a queue using the supplied redis DB', done => {
      const queue = new Queue('custom', { redis: { DB: 1 } } as any);
    ...
    it('creates a queue using the supplied redis host', done => {
      ...
      expect((queue.client as any).options.host).to.be.equal('localhost');
    ...
    it('creates a queue accepting port as a string', () => {
      const queue = new Queue('foobar', '6379', 'localhost' as any);
    ...
    // TODO createClient() is not supported in BullMQ
    // it('should allow reuse redis connections', done => {
    //   const client = new IORedis();
    //   const subscriber = new IORedis();
    //
    //   const opts = {
    //     createClient(type: string, opts: IORedis.RedisOptions) {
    //       switch (type) {
    //         case 'client':
    //           return client;
    //         case 'subscriber':
    ...
    //         expect(job.id).to.be.ok;
    //         expect(job.data.qux).to.be.equal('baz');
    //         let completed = 0;
    //
  ...
  describe(' a worker', () => {
    ...
    it('process a job that updates progress', done => {
      ...
      queue.on('progress', (job, progress) => {
        expect(job).to.be.ok;
        expect(progress).to.be.equal(42);
        done();
      });
    });
    ...
    it('process a job that returns a promise', done => {
      ...
      queue.on('completed', (job, data) => {
        expect(job).to.be.ok;
        expect(data).to.be.equal('my data');
        done();
      });
    });
    ...
    //                       try {
    //                         expect(stalledCallback.calledOnce).to.be.equal(true);
    ...
    it('process a two named jobs that returns a promise', done => {
      ...
      queue.on('completed', (job, data) => {
        ...
        if (job.data.baz) {
          two = true;
          expect(data).to.be.equal('my data 2');
        }
        if (one && two) {
          done();
        }
    ...
    //   Promise.all(jobs).then(() => {
    //     let processed = 0;
    //     const procFn = function(this: Queue) {
    ...
    //     expect(job['failedReason']).to.be.equal(FAILED_MESSAGE);
    ...
    it('process a job that fails', done => {
      ...
      queue.once('failed', (job, err) => {
        expect(job.id).to.be.ok;
        expect(job.data.foo).to.be.equal('bar');
        expect(err).to.be.equal(jobError);
        done();
      });
    });
    ...
    it('process a job that rejects with a nested error', done => {
      ...
      const jobError: any = new Error('wrapper');
      ...
      queue.once('failed', (job, err) => {
        ...
        expect(job.data.foo).to.be.equal('bar');
        expect(err).to.be.equal(jobError);
        expect(err.cause()).to.be.equal(cause);
        done();
      });
    });
    ...
    it.skip('does not renew a job lock after the lock has been released [#397]', function() {
      this.timeout(LOCK_RENEW_TIME * 4);
      ...
      queue.emit = function(event: string | symbol, ...args: any[]): boolean {
        utils.sleep(LOCK_RENEW_TIME * 2).then(() => {
          emit.apply(null, arguments as any);
        });
        return true;
      };

      setTimeout(queue.close.bind(queue), LOCK_RENEW_TIME * 2.5);
    ...
    //     expect(job).to.be.ok;
    //     expect(job.data.foo).to.be.equal('bar');
    //     expect(err).to.be.equal(notEvenErr);
    //     failedOnce = true;
    //     retryQueue.retryJob(job);
    //   });
    //
    //   retryQueue.once('completed', () => {
    //     expect(failedOnce).to.be.equal(true);
  ...
  // TODO queue.empty() not supported
  ...
  //     .then(count => {
  //       expect(count).to.be.equal(maxJobs);
  //     })
  ...
  describe('Delayed jobs', () => {
    ...
    //       })
    //       .then(() => {
    //         expect(publishHappened).to.be.equal(true);
    ...
    //       expect(job.id).to.be.ok;
    //       expect(job.data.delayed).to.be.equal('foobar');
    //       expect(job['delay']).to.be.equal(delay);
    //     });
    //   });
    // });
    ...
    //
    //   const fn = function(job: Job, jobDone: Function) {
    //     expect(order).to.be.equal(job.data.order);
    //     jobDone();
    //
    //     if (order === 4) {
    ...
    //
    //   queue.on('error', (/*err*/) => {
    //     job.isDelayed().then((isDelayed: boolean) => {
    ...
    //
    //   queue.on('error', (/*err*/) => {
    //     job.isWaiting().then((isWaiting: boolean) => {
  ...
  describe('Retries and backoffs', () => {
    ...
    it('should automatically retry a failed job if attempts is bigger than 1', done => {
      ...
      queue.isReady().then(() => {
        let tries = 0;
        queue.process((job, jobDone) => {
          expect(job.attemptsMade).to.be.equal(tries);
          tries++;
    ...
    it('should not retry a failed job more than the number of given attempts times', done => {
      ...
      queue.isReady().then(() => {
        queue.process((job, jobDone) => {
          ...
          expect(job.attemptsMade).to.be.equal(tries - 1);
          jobDone();
        });
  ...
  describe('Cleaner', () => {
    ...
    //     expect(type).to.be.equal('completed');
    //     expect(jobs.length).to.be.equal(0);
    //     utils.cleanupQueue(testQueue);
    //     done();
    //   });
