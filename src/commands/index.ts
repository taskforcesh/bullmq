...
function getCallerFile() {
  ...
  try {
    ...
    const sites = <NodeJS.CallSite[]>(<unknown>err.stack);
...
function interpolate(file: ScriptInfo, cache?: Set<string>): string {
  ...
  let content = file.content;
  file.includes.forEach((child: ScriptInfo) => {
    const emitted = cache.has(child.path);
    const fragment = interpolate(child, cache);
