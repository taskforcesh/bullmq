defmodule BullMQ.Scripts do
  ...
  def add_standard_job(conn, ctx, job, opts) do
    ...
    keys = [
      Keys.wait(ctx),          # KEYS[1] 'wait'
      Keys.paused(ctx),        # KEYS[2] 'paused'
      Keys.meta(ctx),          # KEYS[3] 'meta'
      Keys.id(ctx),            # KEYS[4] 'id'
      Keys.completed(ctx),     # KEYS[5] 'completed'
      Keys.delayed(ctx),       # KEYS[6] 'delayed'
      Keys.active(ctx),        # KEYS[7] 'active'
      Keys.events(ctx),        # KEYS[8] events stream key
      Keys.marker(ctx)         # KEYS[9] marker key
    ]
    ...
    packed_args = Msgpax.pack!([
      Keys.key_prefix(ctx),    # [1] key prefix (with trailing colon for job key building)
      job_id || "",            # [2] custom id (empty = auto generate)
      job_name,                # [3] name
      timestamp,               # [4] timestamp
      parent_key,              # [5] parentKey?
      parent_deps_key,         # [6] parent dependencies key
      parent,                  # [7] parent? {id, queueKey}
      repeat_job_key,          # [8] repeat job key
      deduplication_key        # [9] deduplication key
    ], iodata: false)
  ...
  def add_standard_jobs_pipelined(conn, ctx, jobs_with_opts) do
    ...
    # Build all commands
    commands = Enum.map(jobs_with_opts, fn {job, opts} ->
  ...
  def add_delayed_job(conn, ctx, job, opts) do
    # KEYS in order expected by Lua script
    keys = [
      Keys.marker(ctx),        # KEYS[1] 'marker'
      Keys.meta(ctx),          # KEYS[2] 'meta'
      Keys.id(ctx),            # KEYS[3] 'id'
      Keys.delayed(ctx),       # KEYS[4] 'delayed'
      Keys.completed(ctx),     # KEYS[5] 'completed'
      Keys.events(ctx)         # KEYS[6] events stream key
    ]
    ...
    # ARGV[1] msgpacked arguments array
    packed_args = Msgpax.pack!([
      Keys.key_prefix(ctx),    # [1] key prefix (with trailing colon)
      job_id || "",            # [2] custom id (empty = auto generate)
      job_name,                # [3] name
      delayed_timestamp,       # [4] delayed timestamp
      parent_key,              # [5] parentKey?
      parent_deps_key,         # [6] parent dependencies key
      parent,                  # [7] parent? {id, queueKey}
      repeat_job_key,          # [8] repeat job key
  ...
  def add_prioritized_job(conn, ctx, job, opts) do
    ...
    keys = [
      ...
      Keys.meta(ctx),          # KEYS[2] 'meta'
      Keys.id(ctx),            # KEYS[3] 'id'
      Keys.prioritized(ctx),   # KEYS[4] 'prioritized'
      Keys.delayed(ctx),       # KEYS[5] 'delayed'
      Keys.completed(ctx),     # KEYS[6] 'completed'
      Keys.active(ctx),        # KEYS[7] 'active'
      Keys.events(ctx),        # KEYS[8] events stream key
      Keys.pc(ctx)             # KEYS[9] 'pc' priority counter
    ]
  ...
  def build_add_parent_job_command(ctx, job, opts) do
    keys = [
      Keys.meta(ctx),              # KEYS[1] 'meta'
      Keys.id(ctx),                # KEYS[2] 'id'
      Keys.delayed(ctx),           # KEYS[3] 'delayed'
      Keys.waiting_children(ctx),  # KEYS[4] 'waiting-children'
      Keys.completed(ctx),         # KEYS[5] 'completed'
      Keys.events(ctx)             # KEYS[6] events stream key
    ]
  ...
  def move_to_active(conn, ctx, token, opts \\ []) do
    # Keys in order expected by Lua script
    keys = [
      Keys.wait(ctx),          # KEYS[1] wait key
      Keys.active(ctx),        # KEYS[2] active key
      Keys.prioritized(ctx),   # KEYS[3] prioritized key
      Keys.events(ctx),        # KEYS[4] stream events key
      Keys.stalled(ctx),       # KEYS[5] stalled key
      Keys.limiter(ctx),       # KEYS[6] rate limiter key
      Keys.delayed(ctx),       # KEYS[7] delayed key
      Keys.paused(ctx),        # KEYS[8] paused key
      Keys.meta(ctx),          # KEYS[9] meta key
      Keys.pc(ctx),            # KEYS[10] pc priority counter
      Keys.marker(ctx)         # KEYS[11] marker key
    ]
    ...
    args = [
      Keys.key_prefix(ctx),   # ARGV[1] key prefix (with trailing colon)
      timestamp,              # ARGV[2] timestamp
      packed_opts             # ARGV[3] msgpacked opts
    ]
  ...
  def move_to_finished(conn, ctx, job_id, token, result, target, opts \\ []) do
    ...
    keys = [
      ...
      Keys.active(ctx),        # KEYS[2] active key
      Keys.prioritized(ctx),   # KEYS[3] prioritized key
      Keys.events(ctx),        # KEYS[4] event stream key
      Keys.stalled(ctx),       # KEYS[5] stalled key
      Keys.limiter(ctx),       # KEYS[6] rate limiter key
      Keys.delayed(ctx),       # KEYS[7] delayed key
      Keys.paused(ctx),        # KEYS[8] paused key
      Keys.meta(ctx),          # KEYS[9] meta key
      Keys.pc(ctx),            # KEYS[10] pc priority counter
      Keys.get(ctx, target_str), # KEYS[11] completed/failed key
      Keys.job(ctx, job_id),   # KEYS[12] jobId key
      metrics_key,             # KEYS[13] metrics key
      Keys.marker(ctx)         # KEYS[14] marker key
    ]
    ...
    args = [
      job_id,                  # ARGV[1] jobId
      timestamp,               # ARGV[2] timestamp
      prop_val,                # ARGV[3] msg property returnvalue / failedReason
      result_val,              # ARGV[4] return value / failed reason
      target_str,              # ARGV[5] target (completed/failed)
      fetch_next,              # ARGV[6] fetch next?
      Keys.key_prefix(ctx),    # ARGV[7] keys prefix (with trailing colon)
      packed_opts,             # ARGV[8] opts
      fields_to_update         # ARGV[9] job fields to update (for stacktrace)
    ]
  ...
  def extend_locks(conn, ctx, job_ids, tokens, duration) when is_list(job_ids) and is_list(tokens) do
    keys = [
      Keys.stalled(ctx)
    ]
  ...
  def move_stalled_jobs_to_wait(conn, ctx, max_stalled_count, opts \\ []) do
    ...
    args = [
      Keys.key(ctx),          # ARGV[1] prefix:queueName
      max_stalled_count,      # ARGV[2] max stalled count
      timestamp               # ARGV[3] timestamp
    ]
  ...
  def retry_job(conn, ctx, job_id, lifo, token) do
    ...
    args = [
      Keys.key(ctx),          # ARGV[1] prefix:queueName
      job_id,                 # ARGV[2] job id
      if(lifo, do: "LIFO", else: "FIFO"),  # ARGV[3] push type
      token                   # ARGV[4] worker token
    ]
  ...
  def drain(conn, ctx, delayed?) do
    ...
    args = [
      Keys.key(ctx),          # ARGV[1] prefix:queueName
      if(delayed?, do: 1, else: 0)  # ARGV[2] include delayed
    ]
  ...
  def remove_job(conn, ctx, job_id, remove_children) do
    keys = [
      Keys.job(ctx, job_id),      # KEYS[1] jobKey
      Keys.repeat(ctx)            # KEYS[2] repeat key
    ]

    args = [
      job_id,                               # ARGV[1] jobId
      if(remove_children, do: "1", else: "0"), # ARGV[2] remove children
      Keys.key(ctx) <> ":"                  # ARGV[3] queue prefix (with trailing colon)
    ]
  ...
  def update_job_scheduler(conn, ctx, scheduler_id, next_millis, template_data, job_opts, producer_id) do
    # KEYS in order expected by Lua script (updateJobScheduler-12.lua)
    keys = [
      Keys.repeat(ctx),         # KEYS[1] 'repeat' key
      Keys.delayed(ctx),        # KEYS[2] 'delayed'
      Keys.wait(ctx),           # KEYS[3] 'wait' key
      Keys.paused(ctx),         # KEYS[4] 'paused' key
      Keys.meta(ctx),           # KEYS[5] 'meta'
      Keys.prioritized(ctx),    # KEYS[6] 'prioritized' key
      Keys.marker(ctx),         # KEYS[7] 'marker'
      Keys.id(ctx),             # KEYS[8] 'id'
      Keys.events(ctx),         # KEYS[9] events stream key
      Keys.pc(ctx),             # KEYS[10] 'pc' priority counter
      Keys.job(ctx, producer_id), # KEYS[11] producer key
      Keys.active(ctx)          # KEYS[12] 'active' key
    ]

    timestamp = System.system_time(:millisecond)

    args = [
      next_millis,              # ARGV[1] next milliseconds
      scheduler_id,             # ARGV[2] jobs scheduler id
      template_data,            # ARGV[3] Json stringified delayed data
      job_opts,                 # ARGV[4] msgpacked delayed opts
      timestamp,                # ARGV[5] timestamp
      Keys.key_prefix(ctx),     # ARGV[6] prefix key
      producer_id               # ARGV[7] producer id
    ]
  ...
  def get_metrics(conn, ctx, type, start_idx \\ 0, end_idx \\ -1) do
    ...
    keys = [
      Keys.metrics(ctx, type_str),           # KEYS[1] metrics key
      "#{Keys.metrics(ctx, type_str)}:data"  # KEYS[2] metrics data key
    ]

    args = [
      start_idx,    # ARGV[1] start index
      end_idx       # ARGV[2] end index
    ]
  ...
  defp get_job_timestamp(job) when is_map(job), do: Map.get(job, :timestamp) || Map.get(job, "timestamp")
  ...
  defp get_job_priority(job) when is_map(job), do: Map.get(job, :priority) || Map.get(job, "priority") || 0
  defp get_job_priority(_), do: 0

  defp get_repeat_job_key(job) when is_struct(job), do: Map.get(job, :repeat_job_key)
  defp get_repeat_job_key(job) when is_map(job), do: Map.get(job, :repeat_job_key) || Map.get(job, "repeatJobKey")
  ...
  def pack_job_opts(job, additional_opts \\ %{}) do
    ...
    |> maybe_add_opt("pri", Map.get(job_opts, :priority) || Map.get(job_opts, "priority") || priority, 0)
    ...
    |> maybe_add_opt("ro", Map.get(job_opts, :remove_on_complete) || Map.get(job_opts, "removeOnComplete"), nil)
    |> maybe_add_opt("rof", Map.get(job_opts, :remove_on_fail) || Map.get(job_opts, "removeOnFail"), nil)
    |> maybe_add_opt("fpof", Map.get(job_opts, :fail_parent_on_failure) || Map.get(job_opts, "failParentOnFailure"), nil)
    |> maybe_add_opt("idof", Map.get(job_opts, :ignore_dependency_on_failure) || Map.get(job_opts, "ignoreDependencyOnFailure"), nil)
    |> maybe_add_opt("rdof", Map.get(job_opts, :remove_dependency_on_failure) || Map.get(job_opts, "removeDependencyOnFailure"), nil)
  ...
  defp encode_arg(arg) when is_atom(arg) and not is_nil(arg) and not is_boolean(arg), do: Atom.to_string(arg)
