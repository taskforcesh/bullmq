defmodule BullMQ.JobSchedulerStressTest do
  ...
  describe "rapid upserts" do
    @tag :stress
    test "handles many rapid upserts maintaining single scheduler", %{conn: conn, queue_name: queue_name} do
    ...
    @tag :stress
    test "concurrent upserts with different IDs create separate schedulers", %{conn: conn, queue_name: queue_name} do
  ...
  describe "cron pattern stress" do
    ...
    test "handles many different cron patterns", %{conn: conn, queue_name: queue_name} do
      ...
      for {pattern, idx} <- Enum.with_index(patterns) do
        {:ok, scheduler} = JobScheduler.get(conn, queue_name, "pattern-#{idx}", prefix: @test_prefix)
  ...
  describe "edge cases" do
    ...
    test "handles unicode in job data", %{conn: conn, queue_name: queue_name} do
      ...
      {:ok, scheduler} = JobScheduler.get(conn, queue_name, "unicode-scheduler", prefix: @test_prefix)
    ...
    test "handles large job data", %{conn: conn, queue_name: queue_name} do
      ...
      {:ok, scheduler} = JobScheduler.get(conn, queue_name, "large-data-scheduler", prefix: @test_prefix)
