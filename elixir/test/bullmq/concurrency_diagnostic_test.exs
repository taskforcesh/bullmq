defmodule BullMQ.ConcurrencyDiagnosticTest do
  ...
  test "verify concurrency with slow jobs", %{queue_name: queue_name, conn: conn} do
    ...
    {:ok, tracker} = Agent.start_link(fn -> %{
    ...
    # Assertions
    assert stats.max_concurrent > 1, "Jobs should run in parallel, but max_concurrent was #{stats.max_concurrent}"
  ...
  test "compare throughput with different concurrency levels", %{queue_name: queue_name, conn: conn} do
    ...
    results = for concurrency <- [1, 5, 10, 20] do
      ...
      IO.puts("Concurrency #{concurrency}: #{count} jobs in #{elapsed}ms = #{throughput} jobs/sec")
