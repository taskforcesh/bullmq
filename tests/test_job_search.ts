    ...
    describe('$indexOfBytes: invalid parameters', () => {
      async function runit(
        ...
        if (expected) {
          ...
          expect(() => checkExpression(criteria, 0)).to.throw();
        }
      }
      ...
      async function testIndices(
        ...
        return runit('$data.haystack', 'ignored', start, end, expected);
      }

      it('throws if the haystack is not a string', async () => {
        await runit(18, '', 0, 1000, /expected a string as the first argument/);
      });
